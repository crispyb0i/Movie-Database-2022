...
export const fetchMovieCreditsByID = (media_id) => {
	return fetch(
		`${BASE_URL}/movie/${media_id}/credits?api_key=${API_KEY}`
	).then((res) => res.json());
};
...
export const fetchTVCreditsByID = (media_id) => {
	return fetch(
		`${BASE_URL}/tv/${media_id}/credits?api_key=${API_KEY}`
	).then((res) => res.json());
};
...
export const fetchSearchResults = (query) => {
	return fetch(
		`https://api.themoviedb.org/3/search/multi?api_key=${BASE_URL}&language=en-US&query=${query}&page=1&include_adult=false`
	).then((res) => res.json());
};
